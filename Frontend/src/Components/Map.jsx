...
const Maps = () => {
  ...
  useEffect(() => {
    ...
    // Load obstacle data
    fetch("/obstacles.geojson") // Load from public folder
      .then((response) => response.json())
      .then((data) => {
        setObstacles(data.features);
        data.features.forEach((obstacle) => {
          new mapboxgl.Marker({ color: "#ff0000" })
            .setLngLat(obstacle.geometry.coordinates)
            .setPopup(new mapboxgl.Popup().setHTML(`<p>${obstacle.properties.description}</p>`))
            .addTo(mapInstance);
        });
      });
